...
namespace NewDemoInvoiceApplication.Controllers
{
    ...
    public class InvoiceItemController : ControllerBase
    {
        ...
        [HttpGet("GetInvoiceItemByInvoiceNo")]
        public async Task<IActionResult> GetInvoiceItemByIinvoiceNo(int invoice_no)
        {
            BaseResponseModel baseResponse = new BaseResponseModel();
            try
            {
                var result = await invoiceAsyc.GetInvoiceDataById(invoice_no);
                if (result != null)
                {
        ...
        public async Task<IActionResult> DeleteInvoiceItem(int invoice_no)
        {
            BaseResponseModel baseResponse = new BaseResponseModel();
            try
            {
                var result = await invoiceAsyc.DeleteInvoiceData(invoice_no);

                if (result > 0)
                {
                    baseResponse.SuccessMessage = "Invoice item is deleted successfully...!";
                    baseResponse.ResponseData = invoice_no;
                    return Ok(result);
                }
                else
                {
